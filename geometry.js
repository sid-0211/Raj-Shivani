google.maps.__gjsload__("geometry", function (_) {
  var oqa = function (a, b) {
    ...
    pqa = function (a, b, c, d, e) {
      if (!d) {
        c = oqa(a.lng(), c) / oqa(a.lng(), b.lng());
        if (!e)
          return (
            (e = Math.sin(_.Fl(a.lat()))),
            (e = Math.log((1 + e) / (1 - e)) / 2),
            (b = Math.sin(_.Fl(b.lat()))),
            _.Gl(
              2 *
                Math.atan(
        ...
        return e
          .fromPointToLatLng(
            new _.Qo(a.x + c * (b.x - a.x), a.y + c * (b.y - a.y)),
          )
          .lat();
      }
      e = _.Fl(a.lat());
    ...
    rqa = function (a, b, c) {
      ...
      for (let q = 0, u = g.getLength(); q < u; ++q) {
        ...
        for (let F = 0; F < y; ++F) {
          var h = v.getAt(F);
          let I = v.getAt((F + 1) % x);
          if (qqa(h, a) || qqa(I, a)) return !0;
          var k = _.ym(h.lng(), -180, 180),
            m = _.ym(I.lng(), -180, 180);
          let O = Math.max(k, m),
            L = Math.min(k, m);
          if (
            (k =
              Math.abs(_.ym(k - m, -180, 180)) <= 1e-9 &&
              (Math.abs(_.ym(k - d, -180, 180)) <= c ||
                Math.abs(_.ym(m - d, -180, 180)) <= c))
          ) {
            k = a.lat();
            m = Math.min(h.lat(), I.lat()) - c;
            var p = Math.max(h.lat(), I.lat()) + c;
            k = k >= m && k <= p;
          }
          if (k) return !0;
          if (O - L > 180 ? d + c >= O || d - c <= L : d + c >= L && d - c <= O)
            if (((h = pqa(h, I, d, f, b)), Math.abs(h - a.lat()) < c))
              return !0;
        }
      }
      return !1;
    ...
    wqa = function (a, b) {
      b = b || 6378137;
      a instanceof _.Bp && (a = a.getArray());
      a = (0, _.dt)(a);
      ...
      for (let v = 0; v < a.length; ++v)
        (uqa(a[v], d), MC(f, d, c), vqa(c, e, e), ([f[0], f[1], f[2]] = d));
      var [g, h, k] = f,
        [m, p, q, u] = e;
    ...
    XC = class {};
  XC.isLocationOnEdge = rqa;
  XC.containsLocation = function (a, b) {
    a = _.zn(a);
    var c = _.ym(a.lng(), -180, 180),
      d = !!b.get("geodesic"),
      e = b.get("latLngs"),
      f = b.get("map");
    ...
    for (let k = 0, m = e.getLength(); k < m; ++k) {
      ...
      for (let q = 0, u = p.getLength(); q < u; ++q) {
        ...
        let y = _.ym(x.lng(), -180, 180),
          F = Math.max(h, y);
        h = Math.min(h, y);
        (F - h > 180 ? c >= F || c < h : c < F && c >= h) &&
          pqa(v, x, c, d, f) < a.lat() &&
          (g = !g);
      }
    }
  ...
  var YC = class {};
  YC.computeSignedArea = wqa;
  YC.computeArea = function (a, b) {
    if (
      !(
        a instanceof _.Bp ||
        Array.isArray(a) ||
    ...
    if (a instanceof _.Do) {
      ...
      let c =
        2 * Math.PI * b ** 2 * (1 - Math.cos(((a.ij.lo - 90) * Math.PI) / 180));
      c -=
        2 * Math.PI * b ** 2 * (1 - Math.cos(((a.ij.hi - 90) * Math.PI) / 180));
      return (c * Math.abs(a.Ai.hi - a.Ai.lo)) / 360;
    }
    return Math.abs(wqa(a, b));
  };
  YC.computeLength = function (a, b) {
    b = b || 6378137;
    var c = 0;
    a instanceof _.Bp && (a = a.getArray());
    for (let d = 0, e = a.length - 1; d < e; ++d) c += tqa(a[d], a[d + 1], b);
    return c;
  };
  YC.computeDistanceBetween = tqa;
  YC.traversePath = function () {
    throw Error(
      "traversePath() is not available in this version of the Maps JavaScript API.",
    );
  };
  YC.interpolate = function (a, b, c) {
    a = _.zn(a);
    b = _.zn(b);
    var d = _.vn(a),
      e = _.wn(a),
    ...
    b = sqa(a, b);
    var m = Math.sin(b);
    if (m < 1e-6) return new _.tn(a.lat(), a.lng());
    a = Math.sin((1 - c) * b) / m;
    c = Math.sin(c * b) / m;
  ...
  YC.computeOffsetOrigin = function (a, b, c, d) {
    a = _.zn(a);
    c = _.Fl(c);
    b /= d || 6378137;
    d = Math.cos(b);
    ...
    a = _.wn(a) - Math.atan2(b, d * Math.cos(g) - e * Math.sin(g));
    return new _.tn(_.Gl(g), _.Gl(a));
  };
  YC.computeOffset = function (a, b, c, d) {
    a = _.zn(a);
    b /= d || 6378137;
    c = _.Fl(c);
    var e = _.vn(a);
    ...
    return new _.tn(
      _.Gl(Math.asin(g)),
      _.Gl(a + Math.atan2(b * e * Math.sin(c), d - f * g)),
    );
  };
  YC.computeHeading = function (a, b) {
    a = _.zn(a);
    b = _.zn(b);
    var c = _.vn(a),
      d = _.wn(a);
  ...
  var yqa = { encoding: _.Ou, spherical: YC, poly: XC };
  _.ya.google.maps.geometry = yqa;
  _.bm("geometry", yqa);
});
